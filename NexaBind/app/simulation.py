...
class Simulation(object):
    ...
    def __init__(self, topology, system, platform=None, platformProperties=None, state=None):
        """Create a Simulation.
        ...
        self.context="The Simulation.context class is under construction"
        #self.context="The Simulation.context.setPositions class is under construction"
        self.minimizeEnergy="The Simulation.minimizeEnergy class is under construction"
        self.reporters="The Simulation.reporters class is under construction"
        #self.reporters.append(StateDataReporter(stdout, 1000, step=True, potentialEnergy=True, temperature=True))
        self.step="The Simulation.step class is under construction"
    ...
    def runForClockTime(self, time, checkpointFile=None, stateFile=None, checkpointInterval=None):#Note24: looks extra for now
    ...
    def _simulate(self, endStep=None, endTime=None):
        ...
        while self.currentStep < endStep and (endTime is None or datetime.now() < endTime):
            ...
            #Note24: try to print nextSteps see why it's written this way
            
            # Find when the next report will happen.
            
            anyReport = False
            for i, reporter in enumerate(self.reporters):
                nextReport[i] = reporter.describeNextReport(self) #Note24: reporter is initiated in the __init__() self.reporters = [] 
    ...
    def _generate_reports(self, reports, periodic):#Note24: check what's reports type and data structure here as input 
        getPositions = False
        getVelocities = False
        getForces = False
        getEnergy = False
    ...
    def saveCheckpoint(self, file):#Note24: essentially is used to resume the sim somewhere else, another option is saveState()
        ...
        file : string or file
            ...
            filename
        """
        if isinstance(file, str): #Note24: example Check if the number 5 is an integer: x = isinstance(5, int)
            with open(file, 'wb') as f:#Note24: The wb indicates that the file is opened for writing in binary mode.
                f.write(self.context.createCheckpoint())
        else:
            file.write(self.context.createCheckpoint())

    def loadCheckpoint(self, file):#Note24: imp file can be string (filename) or file
        ...
        if isinstance(file, str):
            with open(file, 'rb') as f:# Note24: rb: Opens the file as read-only in binary format and starts reading from the beginning of the file.
