...
import sys
from collections import OrderedDict #Note24: OrderedDict is a dict subclass that preserves the order in which key-value pairs, commonly known as items, are inserted into the dictionary.
from NexaBind.unit import daltons, is_quantity
...
class Element(object):
    ...
    The NexaBind.app.element module contains objects for all the standard chemical elements,
    ...
    @staticmethod #Note24: Self is used to inform that function should work on the instance of the class(object), if the function doesn’t have self same as here it include it inside the class as the programmer feels it pairs well with the class you just need to add @staticmethod above the class, it means if this method was outside of the class no error would occur it just cleaner and also paired to this class so I use it here.
